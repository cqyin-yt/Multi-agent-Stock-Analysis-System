...
import { AgentConfig, AgentRole, ModelProvider } from '../types';
...
const AgentCard: React.FC<AgentCardProps> = ({ config, content, isLoading, isPending, isConfigMode, onConfigChange }) => {
  ...
  // 根据配置判断使用的模型
  const isGeminiAgent = config.modelProvider === ModelProvider.GEMINI;
  const isVolcanoAgent = config.modelProvider === ModelProvider.VOLCANO;
  
  // 其他智能体使用 DeepSeek
  const isDeepSeekAgent = !isGeminiAgent && !isVolcanoAgent;
        ...
        {!isConfigMode && (
          <div className="ml-auto flex flex-col items-end gap-0.5">
             {isGeminiAgent ? (
                 ...
                 <span className="text-[9px] uppercase font-mono px-1.5 py-0.5 rounded bg-orange-500/20 text-orange-300">火山方舟</span>
             ) : (
        ...
        <div className="p-3 border-b border-white/5 bg-black/20 space-y-3 shrink-0">
           ...
           <div>
             ...
             {isGeminiAgent ? (
                 ...
                 <div className="w-full bg-slate-900/50 text-xs text-slate-400 border border-white/5 rounded px-2 py-1.5 cursor-not-allowed italic flex items-center justify-between">
                    <span>Volcano Ark</span>
                    <span className="text-[9px] bg-orange-500/20 text-orange-300 px-1 rounded">AUTO</span>
                 </div>
             ) : isDeepSeekAgent ? (
                 <div className="w-full bg-slate-900/50 text-xs text-slate-400 border border-white/5 rounded px-2 py-1.5 cursor-not-allowed italic flex items-center justify-between">
